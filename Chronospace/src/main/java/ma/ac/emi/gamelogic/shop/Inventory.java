...
public class Inventory {
    ...
    // Numeric upgrade tracking (existing system — unchanged)
    private List<UpgradeItem> weaponUpgrades;
    private List<UpgradeItem> playerUpgrades;

    // Behavioral effect bus (new system)
    ...
    public void removeItem(ShopItem item) {
        ...
        // Unregister behavioral effect if this item had one
        if (item instanceof UpgradeItem ui && ui.getEffect() != null) {
            // Player reference needed — caller (ShopManager.sellItem) calls
            // recalculateAllUpgrades immediately after, which re-registers
            // everything; so we just remove the effect without firing onUnregister
            // here to avoid needing the Player reference on Inventory.
            // If you want onUnregister, pass Player through removeItem instead.
    ...
    public void init() {
        ...
        // effectContext is final — effects from previous round are cleared via clear()
