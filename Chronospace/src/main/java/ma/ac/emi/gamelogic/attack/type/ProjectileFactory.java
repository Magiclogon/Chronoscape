...
public class ProjectileFactory {
	public static Projectile createProjectile(String id, Vector3D pos, Vector3D dir, double speed, Weapon weapon, Vector3D target) {
		return createProjectile(id, pos, dir, speed, weapon, target, List.of());
	}
	
	public static Projectile createProjectile(String id, Vector3D pos, Vector3D dir, double speed,
			Weapon weapon, Vector3D target, List<Behavior> additionalBehaviors) {
		...
		if(!def.getId().equals("none")) projectile.setSprite(AssetsLoader.getSprite(def.getSpritePath()));
